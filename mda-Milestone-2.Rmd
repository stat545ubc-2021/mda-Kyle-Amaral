...
My data is tidy as can be seen by each column representing a single variable, each row represents an observation, and each cell represents a single value. The below table shows 8 variables from my dataset and as you can see, each cell represents 1 piece of data.
...
```{r}
cancer_sample%>%
  pivot_longer(cols = c(-ID, -diagnosis), names_to =  c("measurements", "mean"), names_sep = "_")
```
...
My original table was presented in a tidy manner as seen by each cell representing one sole piece of information, each row is an observation, and each column is a variable. Therefore, to make this table untidy, I utilized the pivot_longer function to decrease the number of variables. By doing this it made one of the columns represent more than one variable.
